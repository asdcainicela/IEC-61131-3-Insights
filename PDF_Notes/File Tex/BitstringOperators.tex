...
%\begin{enumerate}
	%\item 
	Check if the pointer \highlight{pSensor} is not null. If it is null, the rest of the condition is not evaluated.
	%\item
	 The \highlight{pxSensor^} is only evaluated if \highlight{pxSensor <> 0} is \highlight{TRUE}, avoiding dereference errors.
%\end{enumerate}

Using \highlight{AND\_THEN} prevents the program from accessing \highlight{psSensor^} when \highlight{pxSensor} is 0, avoiding runtime errors, making it safer and more efficient than using \highlight{AND}.
\subsection{OR\_ELSE} 
The operator is an extension of the IEC 61131-3 standard, used
